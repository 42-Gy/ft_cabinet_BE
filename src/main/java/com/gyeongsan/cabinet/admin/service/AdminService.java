...
public class AdminService {
    ...
    public void provideCoin(String username, CoinProvideRequest request) {
        ...
        log.info("[Admin] 유저({})에게 씨앗 {}개 지급. 사유: {}", user.getName(), request.amount(), request.reason());
    }
    ...
    public void forceReturn(Integer visibleNum) {
        ...
        LentHistory activeLent = lentRepository.findByCabinetIdAndEndedAtIsNull(cabinet.getId()).orElse(null);
    ...
    public void updateItemPrice(String itemName, Long newPrice) {
        ...
        log.info("[Admin] 아이템({}) 가격 변경 완료: {} -> {}", item.getName(), oldPrice, newPrice);
    }
    ...
    public void grantItem(String username, ItemGrantRequest request) {
        ...
        log.info("[Admin] 유저({})에게 아이템({}) {}개 지급. 사유: {}", username, request.itemName(), quantity, request.reason());
    }
