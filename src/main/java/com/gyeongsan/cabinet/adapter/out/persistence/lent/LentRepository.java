...
public interface LentRepository extends JpaRepository<LentHistory, Long> {
    ...
    @Query(
            "SELECT lh FROM LentHistory lh JOIN FETCH lh.cabinet JOIN FETCH lh.user "
    ...
    @Query(
            "SELECT lh FROM LentHistory lh "
                    + "JOIN FETCH lh.user u "
                    + "JOIN lh.cabinet c "
                    + "WHERE c.visibleNum = :visibleNum "
                    + "ORDER BY lh.startedAt DESC")
    Page<LentHistory> findHistoryByCabinet(
            @Param("visibleNum") Integer visibleNum, Pageable pageable);
