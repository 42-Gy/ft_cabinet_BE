...
public interface WatermelonJpaRepository extends JpaRepository<WatermelonEntity, Long> {
    ...
    @Query(
            "SELECT w FROM WatermelonEntity w ORDER BY w.highestLevel DESC, w.highestLevelAchievedAt ASC, w.totalAttempts ASC")
    Page<WatermelonEntity> findAllOrderByRanking(Pageable pageable);

    @Query(
            "SELECT COUNT(w) + 1 FROM WatermelonEntity w WHERE "
    ...
    long countBetterThan(
            @Param("highestLevel") int highestLevel,
