package com.gyeongsan.cabinet.adapter.in.scheduler.lent;
...
public class LentScheduler {
    ...
    public void autoExtension() {
        ...
        List<LentHistory> expiringLents =
    ...
    @SchedulerLock(
            name = "monthlyAutoExtensionRetryTask",
            lockAtMostFor = "10m",
            lockAtLeastFor = "1m")
    @Transactional
    public void monthlyAutoExtensionRetry() {
        ...
        List<LentHistory> recentExpiredLents =
                lentRepository.findRecentExpiredActiveLents(lastMonthEnd, now);
        ...
        for (LentHistory lent : recentExpiredLents) {
            ...
            if (!tickets.isEmpty()) {
                ...
                successCount++;
                log.info(
                        "[Monthly Retry] 자동 연장 성공: User={}, NewExpiry={}",
                        user.getName(),
                        newExpiredAt);
            } else {
                log.warn("[Monthly Retry] 자동 연장 실패 (대여권 없음): User={}", user.getName());
            }
        }
    ...
    public void checkOverdue() {
        ...
        for (LentHistory lh : overdueLents) {
            ...
            LocalDateTime gracePeriodEnd = lh.getExpiredAt().toLocalDate().atTime(23, 59, 59);
            ...
            long overdueDays =
                    ChronoUnit.DAYS.between(lh.getExpiredAt().toLocalDate(), now.toLocalDate());
    ...
    @SchedulerLock(
            name = "checkExpirationImminentTask",
            lockAtMostFor = "10m",
            lockAtLeastFor = "1m")
    ...
    private void checkAndSendAlarm(LocalDate targetDate, int daysLeft) {
        ...
        for (LentHistory lh : targetLents) {
            sendImminentAlarm(
                    lh.getUser(), lh.getExpiredAt(), lh.getCabinet().getVisibleNum(), daysLeft);
        }
    ...
    private void sendImminentAlarm(
            User user, LocalDateTime expiredAt, Integer visibleNum, int daysLeft) {
