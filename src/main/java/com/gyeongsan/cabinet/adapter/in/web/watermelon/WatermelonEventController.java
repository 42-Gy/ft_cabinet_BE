...
public class WatermelonEventController {
    ...
    @GetMapping("/me")
    public ApiResponse<WatermelonStatusResponse> getMyStatus(
        ...
        WatermelonStatusResponse response =
                WatermelonStatusResponse.builder()
                        .userId(userId)
    ...
    public ApiResponse<EnhanceResponse> enhance(
        ...
        WatermelonEnhanceResult result =
                enhanceUseCase.enhance(
                        userId,
        ...
        EnhanceResponse response =
                EnhanceResponse.builder()
                        ...
                        .dangerousFertilizerCount(
                                result.getWatermelon().getDangerousFertilizerCount())
                        .build();
    ...
    public ApiResponse<Page<LeaderboardResponse>> getRankings(
        ...
        Page<LeaderboardResponse> response =
                leaderboard.map(
                        wm -> {
                            String name =
                                    userRepository
                                            .findById(wm.getUserId())
                                            .map(User::getName)
                                            .orElse("Unknown");
    ...
    public ApiResponse<List<EnhancementLogResponse>> getLogs() {
        ...
        List<EnhancementLogResponse> response =
                logs.stream()
                        .map(
                                log ->
                                        EnhancementLogResponse.builder()
                                                .id(log.getId())
                                                .userId(log.getUserId())
                                                .userName(
                                                        userIdToNameMap.getOrDefault(
                                                                log.getUserId(), "Unknown"))
                                                .beforeLevel(log.getBeforeLevel())
                                                .afterLevel(log.getAfterLevel())
                                                .usedPremiumFertilizer(
                                                        log.isUsedPremiumFertilizer())
                                                .usedDangerousFertilizer(
                                                        log.isUsedDangerousFertilizer())
                                                .usedDropProtection(log.isUsedDropProtection())
                                                .usedDestroyProtection(
                                                        log.isUsedDestroyProtection())
    ...
    public ApiResponse<WatermelonConfigResponse> getConfig() {
        ...
        WatermelonConfigResponse response =
                WatermelonConfigResponse.builder()
