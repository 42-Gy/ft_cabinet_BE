...
public class StoreDomainService implements StoreUseCase {
    ...
    public void buyItem(Long userId, Long itemId) {
        ...
        log.info(
                "구매 요청 - 유저: {}, 아이템: {}, 가격: {}", user.getName(), item.getName(), item.getPrice());
        ...
        CoinHistory coinUsage =
                CoinHistory.of(user, -item.getPrice(), CoinLogType.ITEM_PURCHASE, item.getName());
    ...
    private void validateExtensionPurchase(Long userId) {
        ...
        int monthlyPurchaseCount =
                itemHistoryRepository.countByUserIdAndItemTypeAndPurchaseAtBetween(
