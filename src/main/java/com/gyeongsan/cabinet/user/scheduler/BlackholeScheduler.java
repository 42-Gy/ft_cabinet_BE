...
public class BlackholeScheduler {
    ...
    public void processBlackholedUsers() {
        ...
        for (User user : blackholedUsers) {
            try {
                // 2. 강제 반납 처리 시도 (LentFacadeService 재활용)
                // 👇 [수정] user.getId() 대신 user 객체 전체를 넘겨야 합니다.
                ...
                slackBotService.sendDm(user.getEmail(), message);

                log.warn("⚫️ {} 유저 강제 반납 및 알림 완료.", user.getName());
