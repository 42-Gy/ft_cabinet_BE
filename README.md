...
src/main/resources/ 경로에 secret.properties 파일을 생성하고 아래 내용을 입력하세요. (주의: DB 비밀번호는 위 .env 파일에서 설정한 값과 일치해야 합니다)
# secret.properties
...
4. 애플리케이션 실행
./gradlew bootRun
서버가 정상적으로 실행되면 http://localhost:8080으로 접속 가능합니다.
